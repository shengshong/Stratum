...
pub fn query(conn: &Connection, term: &str) -> Result<()> {
    ...
    let mut stmt_r = conn.prepare(
        "SELECT subject, predicate, object FROM relations WHERE subject LIKE ?1 OR object LIKE ?1",
    )?;
    let relations: Vec<_> = stmt_r
        .query_map(params![&pattern], |r| {
            Ok((
                r.get::<_, String>(0)?,
                r.get::<_, String>(1)?,
...
pub fn traverse(conn: &Connection, start: &str, max_hops: u32) -> Result<()> {
    println!(
        "{}",
        format!("Graph traversal from '{}' (max {} hops):", start, max_hops).bold()
    );
    ...
    let all_rels: Vec<(String, String, String)> = stmt_all
        .query_map([], |r| {
            Ok((
                r.get::<_, String>(0)?,
                r.get::<_, String>(1)?,
    ...
    while let Some((node, depth)) = queue.pop_front() {
        ...
        for (subj, pred, obj) in &all_rels {
            ...
            if !matches_subj && !matches_obj {
                continue;
            }

            let neighbor = if matches_subj {
                obj.clone()
            } else {
                subj.clone()
            };
            ...
            if !edges_seen
                .iter()
                .any(|(s, p, o, _)| format!("{}\x00{}\x00{}", s, p, o) == edge_key)
            {
    ...
    for node in &visited {
        ...
        let bels: Vec<(String, String, String, f64)> = stmt_bel
            .query_map(params![&pattern], |r| {
    ...
    println!(
        "\n{} node(s) reachable from '{}' (excl. start).",
...
pub fn consolidate(
    ...
    let aging: Vec<(i64, String, String, f64, f64, String)> = stmt_aging
        .query_map([], |r| {
            Ok((
                r.get(0)?,
                r.get(1)?,
    ...
    println!(
        "Aging beliefs (unverified >{} days): {}",
        decay_days,
        aging.len().to_string().yellow()
    );
    ...
    let stale_list: Vec<(String, String, String)> = stmt_stale
    ...
    println!(
        "  Marked stale:     {}",
        if stale_marked > 0 {
            stale_marked.to_string().red().to_string()
        } else {
            "0".to_string()
        }
    );
    ...
    if dry_run {
        println!("  {}", "(DRY RUN — no changes written)".yellow().bold());
    }
...
pub fn status(conn: &Connection) -> Result<()> {
    ...
    let stale: i64 = conn
        .query_row("SELECT COUNT(*) FROM beliefs WHERE stale=1", [], |r| {
            r.get(0)
        })
        .unwrap_or(0);
    if stale > 0 {
        println!(
            "World model: {} entities, {} relations, {} beliefs ({} {})",
            entities,
            relations,
            beliefs,
