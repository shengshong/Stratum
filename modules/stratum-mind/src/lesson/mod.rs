...
pub fn learn(conn: &Connection, content: &str, category: &str, severity: &str, source: Option<&str>) -> Result<i64> {
    conn.execute(
...
pub fn list(conn: &Connection, severity: Option<&str>, category: Option<&str>, resolved: bool, limit: usize) -> Result<()> {
    let mut conditions = vec!["1=1"];
    if !resolved { conditions.push("resolved=0"); }
    let sev_filter;
    if let Some(s) = severity { sev_filter = format!("severity='{}'", s); conditions.push(&sev_filter); }
    let cat_filter;
    if let Some(c) = category { cat_filter = format!("category='{}'", c); conditions.push(&cat_filter); }
    ...
    for (id, content, category, severity, resolved, created_at) in rows {
        ...
        let status = if resolved == 1 { "✓".green() } else { "○".dimmed() };
        let date = &created_at[..10];
        println!("{} [{:>4}] {} {}  {}",
            status, id.to_string().dimmed(),
            sev_color,
            category.dimmed(),
            date.dimmed()
        );
...
pub fn stats(conn: &Connection) -> Result<()> {
    ...
    let resolved: i64 = conn.query_row("SELECT COUNT(*) FROM lessons WHERE resolved=1", [], |r| r.get(0))?;
    let rate = if total > 0 { resolved * 100 / total } else { 0 };
    println!("Lessons: {} total, {} resolved ({}%)", total, resolved, rate);
    Ok(())
}
